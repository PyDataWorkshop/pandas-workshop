...
myDF will be a DataFrame object. You can think of this object holding the contents of the BabyNames in a format similar to a sql table or an excel spreadsheet. Lets take a look below at the contents inside myDF.
...
3	Paul	665
4	David	973
Export the dataframe to a csv file. We can name the file births2016.csv. The function to_csv will be used to export the file. The file will be saved in the same location of the notebook unless specified otherwise.
...
Location = C:\Users\ENTER_USER_NAME.xy\startups\births2016.csv
...
You can think of the numbers [0,1,2,3,4] as the row numbers in an Excel file. In pandas these are part of the index of the dataframe. You can think of the index as the priMarissa key of a sql table with the exception that an index is allowed to have duplicates.
...
#############################################
Prepare Data
The data we have consists of baby names and the number of births in the year 2016. We already know that we have 5 records and none of the records are missing (non-null values).
